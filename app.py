...
with tab_diario:
    ...
    try:
        ...
        if not df_lag.empty:
            ...
            ultima_densidad = df_densidad_valida.iloc[-1]["Densidad"] if not df_densidad_valida.empty else 0.0
            ultimo_peso = df_lag.iloc[-1]["Peso_g"] if not df_lag.empty else 0.0
        else:
            ultima_densidad = 0.0
            ultimo_peso = 0.0
    except:
